...
st.markdown(
    """
#### Для реализации симулятора была выбрана библиотека quiskit от IBM
...
Для наглядности приведем пример работы данной библиотеки на примере програмирования логического сумматора
...
Далее мы могли бы использовать классический XOR, но это читающая операция, которая разрушит суперпозицию кубитов, поэтому вместо этого
используется CNOT (о нем рассказываелось во вкладке "Теоретическое введение")
...
Может показаться, что мы "читаем" в момент `cx` контролирующий, но это не так, тк чтение происходит лишь в момент, когда мы читаем само значение в таргете
...
Таким образом получается схемма сумматора:
...
Для удобство возьмем h с чертой = 1
...
Состояение кубито можно обозначить через матрицы паули
