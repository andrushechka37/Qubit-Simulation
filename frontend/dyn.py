...
st.markdown(
    "**ν_z (частота кубита)** — собственная частота, постоянная часть гамильтониана вдоль Z."
)
...
st.markdown(
    "**ν_x (амплитуда)** — сила драйва; определяет скорость вращения вокруг оси X."
)
st.latex(r"H_{\text{drive}}(t)=(2\pi\nu_x)\cos(2\pi\nu_d t+\varphi)\,X")

st.markdown("**phase (фаза)** — начальная фаза в гармоническом сигнале.")

st.markdown("**duration (длительность)** — время интегрирования.")

st.markdown("**dt** — шаг по времени")

st.markdown("**shots** — число «измерений» для гистограммы.")
